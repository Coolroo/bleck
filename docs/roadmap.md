...
| **A single-file binary** | ✅ `pyinstaller bleck.spec`; CI builds and smoke-tests three platforms, green on all (D83). ✅ Green again on the **pinned** interpreter (D99), which is what proves `setup-uv` fetches `.python-version`'s 3.13 on Linux and macOS and not just where it was developed. ✅ **The tag-triggered job has run, and fully succeeded** — run `#30428190534`, 2026-07-29T06:26:53Z, from sha `747973e`: `checks`, all three platform builds *and* `release` green, publishing four assets. ⛔ The "🔶 has still never run" that sat here was **already wrong when written** (D149). ⚠️ Re-tagging is a different matter: `gh release create` is not idempotent, so pointing the tag at a new commit fails with *"a release with the same tag name already exists"* — and the published assets still correspond to the old commit |
