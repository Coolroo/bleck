/*
    A landmark, so the effects have somewhere to be looked at.

    The first attempt spawned effects at fixed coordinates in a large room and
    nothing was seen -- which cannot distinguish "did not render" from "rendered
    where nobody was looking". This puts a visible object beside the player and
    keeps it there.

    The marker is Mr. L wearing `MOBJ_broken_heart`: repointing a tribe's model
    is one guarded word, proven in D162, and only template 137 uses tribe 295 so
...
#define MAGIC 0x8EA27000u
#define REPORT_WORDS 7
