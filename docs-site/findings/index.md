...
| [Placed items and the coin-flag budget](setup-items.md) | A coin is a save flag from a fixed per-map budget, spent by block coins too — which is why a map that places no floating coins **hangs** if given one |
