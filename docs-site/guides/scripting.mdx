...
Any of the game's several hundred script builtins can be called by name:
...
Names are resolved when the module is built, so a typo is caught then:

```
bleck: the script calls game functions that spm.eu0.lst does not list:
  Missing 1 required symbol(s): evt_mario_no_such_function
...
not ship it. Put it in `symbols/`, or set `BLECK_SYMBOLS_DIR`.
...
|---|---|
| `bleck script check <file>` | Parse and compile; report scripts and functions called |
