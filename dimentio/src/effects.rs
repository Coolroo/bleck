...
///
/// Kept apart from the window so the wrap can be tested with no display: this
/// machine cannot capture its own desktop, so playback that only ran inside a
/// frame callback would have no evidence behind it at all.
...
impl Playback {
    /// Advance `dt` seconds, wrapping at `span` so the effect loops.
    ///
    /// An effect of zero length holds at 0: it is a single frame, and there is
    ...
    /// selection changes, since a new effect has a different length and the
    /// old position may be past its end.
...
mod tests {
    ...
    #[test]
    fn playback_wraps_at_the_effect_duration() {
    ...
    /// A single-frame effect has nothing to scrub, and dividing by its length
    /// would be a division by zero.
    #[test]
    fn a_zero_length_effect_stays_at_the_start() {
        let mut play = Playback {
            time: 0.4,
            playing: true,
        };
